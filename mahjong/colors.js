/* DeetsMahjong — seat-color contract (docs/mahjong.md, "Game palette").

   Pure, DOM-free, dependency-free — engine.js's contract rule applies: the
   Phase-2 worker (../DeetsMahjong) vendors this file VERBATIM, so the mock
   transport and the DO validate a `recolor` byte-identically. Keep it tiny.

   Same contract as DeetsCities' colors.js (the presets, redmean distance,
   and clash check are identical by design — one fewer thing to relearn);
   only the global name differs. Four seats draw from the six presets.

   Browser: window.MahjongColors. Node (worker/self-checks): module.exports. */
(function () {
  "use strict";
  ...
  var API = { PRESETS: PRESETS, MIN_DIST: MIN_DIST,
              norm: norm, dist: dist, clash: clash, freePreset: freePreset };
  if (typeof module !== "undefined" && module.exports) module.exports = API;
  if (typeof window !== "undefined") window.MahjongColors = API;
})();
